...
float4 main(float4 vpos : SV_Position, float2 texcoord : TexCoord) : COLOR
{
	...
	int2 downsample_res;
	if (DOWNSAMPLE_RES) {
		downsample_res = int2(INPUT_X,INPUT_Y);
	}
	else {
		downsample_res = screenSize;
	}

 	color.rgb=CrtsFilter(
  	vpos.xy,
  	downsample_res/screenSize.xy,
  	downsample_res*0.5,
  	1.0/downsample_res,
  	1.0/screenSize.xy,
  	2.0/screenSize.xy,
