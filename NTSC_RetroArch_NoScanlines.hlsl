// MPC-HC port of https://github.com/Matsilagi/RSRetroArch/blob/main/Shaders/NTSC_RetroArch_NoScanlines.fx
...
// uniform float display_sizeX = 320.0;
	// ui_type = "drag";
	// ui_min = 1.0;
	// ui_max = BUFFER_WIDTH;
	// ui_label = "Screen Width (NTSC)";

// uniform float display_sizeY = 240.0;
	// ui_type = "drag";
	// ui_min = 1.0;
	// ui_max = BUFFER_HEIGHT;
	// ui_label = "Screen Height (NTSC)";

uniform bool bUseTwoPhase = true;
	// ui_label = "Use Two-Phase (NTSC)";

uniform bool bUseSVideo = false;
...
float4 NTSCEncodePS(float2 texcoord)
{
	float3 col = tex2D(backbuffer, texcoord).rgb;
...
float4 NTSCDecodePS(float2 texcoord)
{
	...
	signal += tex2D(backbuffer, texcoord).xyz *
...
{
	return NTSCDecodePS(texcoord);	// s0, texcoord
}
